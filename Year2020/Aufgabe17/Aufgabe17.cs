...
internal class Aufgabe17 : IAufgabe
{
    ...
    public string Calc()
    {
        HashSet<Point3D> currentActiveCubes = new();
        ...
        for (int y = 0; y < _input.Length; y++)
        {
            for (int x = 0; x < _input[y].Length; x++)
            {
                ...
                currentActiveCubes.Add(new(x, y, 0));
            }
        }

        for (int i = 0; i < 6; i++)
        {
            DictionaryCounter<Point3D> counter = new(currentActiveCubes.Count * 26);
            ...
            HashSet<Point3D> nextActiveCubes = new(currentActiveCubes.Count);

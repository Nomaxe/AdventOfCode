    ...
    public string Calc()
    {
        for (int i = 0; i < _input.Length; i++)
        {
            for (int j = 0; j < _input.Length; j++)
            {
                ...
                _numbers.Clear();
                Pair node = new(CreatePairNode(_input[i]), CreatePairNode(_input[j]));
                node = Addition(node);

                var magnitude = node.GetMagnitude();
                if (magnitude > _highestMagnitude)
                {
                    _highestMagnitude = magnitude;
                }
            }
        }

        return _highestMagnitude.ToString();
    }

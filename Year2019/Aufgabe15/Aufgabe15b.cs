...
internal class Aufgabe15b : IAufgabe
{
    ...
    private const int StartPosition = 24;
    ...
    public Aufgabe15b()
    {
        ...
        _grid = new(50);
        _position = new(StartPosition, StartPosition);
    }
    ...
    public string Calc()
    {
        ...
        List<Point> points = [oxygenSystem];
        _grid.SetValue(oxygenSystem, 'O');
        int minutes = 0;

        while (points.Count > 0)
        {
            List<Point> nextPoints = new(points.Count);

            foreach (var point in points)
            {
                foreach (var nextPoint in _grid.GetInBoundNeighbours(point).Where(x => _grid.GetValue(x) == '_'))
                {
                    _grid.SetValue(nextPoint, 'O');

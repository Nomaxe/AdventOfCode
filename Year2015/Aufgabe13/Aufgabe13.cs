...
internal class Aufgabe13 : IAufgabe
{
    ...
    public string Calc()
    {
        ...
        int maxHappiness = 0;
        PermuationGenerator<string> generator = new([.. _happiness.Keys]);
        foreach (var permuation in generator.GetPermuations())
        {
            ...
            {
                happiness += _happiness[permuation[i]][permuation[(i + 1) % permuation.Count]];
                happiness += _happiness[permuation[i]][permuation[i > 0 ? i - 1 : permuation.Count - 1]];
            }
